...
const PDF_URL = "https://cloud.nntc.nnov.ru/index.php/s/fYpXD39YccFB5gM/download";
...
function parseSpecialExcelFormat(filePath) {
    ...
    for (let i = 0; i < jsonData.length; i++) {
        ...
        if (dateCell && typeof dateCell === 'string') {
            if (dateCell.includes('понедельник') || 
        ...
        if (groupCell && typeof groupCell === 'string' && (
...
async function saveDataToDb(filePath) {
    try {
        ...
        if (data.length === 0) {
            ...
            const worksheet = workbook.Sheets[sheetName];
            const jsonData = xlsx.utils.sheet_to_json(worksheet, {header: 1});
            console.log("Содержимое файла (первые 10 строк):", jsonData.slice(0, 10));
            return;
        }
        ...
        console.log(`Успешно вставлено ${insertedCount} записей, ошибок: ${errorCount}`);
...
async function checkForChangesAndDownload() {
    ...
    try {
        ...
        const localFileExists = await fs.access(PDF_PATH)
            .then(() => true)
            .catch(() => false);

        if (!localFileExists || (currentLastModified && currentLastModified !== lastModified)) {
            console.log("Обнаружены изменения или файл отсутствует. Скачивание нового файла...");
            ...
            // Проверяем, что XLSX файл создан перед попыткой чтения
            try {
                await fs.access(XLSX_PATH);
                console.log("XLSX файл создан, начинаем обработку...");
                await saveDataToDb(XLSX_PATH);
            } catch (error) {
                console.error("XLSX файл не найден после выполнения Python скрипта:", error.message);
            }
        } else {
...
cron.schedule("*/30 * * * *", checkForChangesAndDownload);
